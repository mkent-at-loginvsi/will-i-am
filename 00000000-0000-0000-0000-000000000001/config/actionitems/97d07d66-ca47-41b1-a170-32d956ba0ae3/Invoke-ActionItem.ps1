...
#>
function Invoke-AMActionItemStopService
{
	[CmdletBinding()]
	param
	(
	...
	$Wait = $($Variables | ? {$_.name -eq "Wait for service to stop"}).Value | Expand-AMEnvironmentVariables
	$Seconds = $($Variables | ? {$_.name -eq "Amount of seconds to wait"}).Value | Expand-AMEnvironmentVariables

	If ($Wait)
	{
		Stop-AMService -Name $ServiceName -Wait -Seconds $Seconds
	}
	else
	{
		Stop-AMService -Name $ServiceName
	}
