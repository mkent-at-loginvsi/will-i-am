...
# Install AM AppStore + Service when needed
$LocalBin = Join-Path $env:am_cache "Bin\appstore"
$CentralBin = Join-Path (Join-Path $env:am_files $AMEnvironment.Id.ToString()) "bin\appstore"
if (-not (Test-Path $LocalBin)) 
{
	[void] (New-Item -ItemType Directory -Path $LocalBin -Force)
}
if ((Test-AMFolderChanges -Source $CentralBin -Destination $LocalBin) -ne 0)
{
	$Service = Get-WmiObject win32_service | ? {$_.name -eq "Login AM App Store Service"}
	Stop-Service "Login AM App Store Service" -ErrorAction SilentlyContinue
	If ($Service -ne $null) {Invoke-Expression "& '$LocalBin\AppStoreService.exe' -uninstall"	}
...
If ($Service -eq $null)
{
	$SA = $AMDataManager.ReadEnvironment($AMEnvironment.Id,$true).ServiceAccount
	Invoke-Expression "& '$LocalBin\AppStoreService.exe' -install -username $($SA.UserName) -password $($SA.Password)"		
}
